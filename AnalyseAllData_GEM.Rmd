...
workDF %>%
  group_by(thisSow, thisHyb) %>%
  mutate(CVperc = sd(TotalBiomass, na.rm = TRUE)/mean(TotalBiomass, na.rm = TRUE)*100) %>%
...
geom_point(aes(shape = thisHyb, size = 20)) 
...
fontSize <- 14
si_df_final %>%
  subset(thisRegion == 3 | thisRegion == 6 | thisRegion == 11 | thisRegion == 12) %>%
...
NZshapeFile <- sf
names(NZshapeFile)
NZshapeFile <- fortify(NZshapeFile, region="REGC2013") # makes it digestible as df to ggplot
plot(NZshapeFile)
```
