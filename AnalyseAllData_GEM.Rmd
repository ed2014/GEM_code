...
# Statistics on G by M effect (CV% among scenarions in eachpixel using 30 year averages)
...
# Total biomass
  ...
  select(-contains("thisSow")) %>%
  select(-contains("thisHyb")) %>%
  summarise(n=n(),
...
mapDf <- workDF %>% 
  select(pixel_ID, thisLat, thisLong, TotalBiomass, HarvestIndex) %>%
  group_by(pixel_ID)  %>% 
  summarise_each(funs(mean))
