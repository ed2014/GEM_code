...
ggsave(file=paste0("TotBio_boxplot.tiff"), dpi = 300)
...
theme(text = element_text(size=25)) + 
theme(legend.position = "none") 
ggsave(file=paste0("HI_boxplot.tiff"), dpi = 300)
...
# save this DF 
write.table(si_df_final_m, file="Si_by_Pixel.txt", row.names=FALSE, col.names=TRUE, sep=" ",append=FALSE, quote=FALSE)
...
si_df_final_m %>%
 ...
 # facet_grid(thisRegion~. , scales="free") +
  ...
  theme(legend.position = "none")
  ggsave(file=paste0("Si_ClimZone_hist.tiff"), dpi = 300)
...
si_df_final_m %>%
 ...
 # ggsave(file="Fig_Ti.tiff", dpi = 300)
  ggsave(file=paste0("Si_bar.tiff"), dpi = 300)

  


# Perc of interaction of each factor
ti_df %>%
ggplot(aes(x=newFactName, y=intPerc)) + 
 geom_bar(width=.5,stat="identity", fill="darkgreen") +
 # coord_flip() + 
 xlab("Factors") + 
 ylab("Share of interaction in factor (%)") +
# scale_x_discrete(labels= newFactName) +
 geom_text(aes(label = paste0(round(ti_df$intPerc, digits=1),"%"), y = mx+3), size = 5)

# reorder factors ascending
ti_df.m$newFactName <- factor(ti_df.m$newFactName, 
  levels = ti_df.m$newFactName[order(ti_df.m$value[ti_df.m$variable == "siMain"])])

mx <- max(ti_df$percTi_norm)

graphTi[met] <- Ti_graph <- ti_df.m %>%
ggplot(aes(x=newFactName, y=normValue, fill=variable, order=rev(variable))) + 
  geom_bar(stat='identity') +
  #coord_flip() +  
  xlab("Main Factors") + ylab("Normalised sums of squares (%)") + 
 # scale_x_discrete(labels= ) + 
  scale_fill_discrete(name="Ti component",
                         labels=c("Interactions", "Main Effects"))+
  theme(text = element_text(size=20)) +
  geom_text(aes(label = paste0("(",round(ti_df.m$percInt, digits=1),"%)"), y = mx+5), size = 7)
 # ggsave(file="Fig_Ti.tiff", dpi = 300)
  ggsave(file=paste0("Fig_Ti_",methodCalc[met], ".tiff"), dpi = 300)
