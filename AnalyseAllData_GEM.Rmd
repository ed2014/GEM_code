...
workDF %>%
  ...
  select(TotalBiomass, HarvestIndex) %>%
  mutate(TotalBiomass_Mg = mean(TotalBiomass)/1000, HI_mean = mean(HarvestIndex)) %>%
  mutate(Bio_CV = ((TotalBiomass_Mg/TotalBiomass_Mg)*100), 
                   HI_CV = ((HI_mean/HI_mean)*100))  %>%
  group_by(Combine,pixel_ID) %>%
  summarise_each(funs(mean)) %>%
  gather(resultVar, value, 5:10) %>% # FIXME: this might change - can you automate?
  filter(resultVar != "TotalBiomass" | resultVar != "HarvestIndex" ) %>%
  ggplot(aes(x=Combine, y=value)) +
  geom_violin() +
  geom_jitter() +
  geom_boxplot(aes(alpha=0.2)) +
  facet_wrap(~resultVar, scales = "free")
...
```{r MapSiBio, echo = FALSE, warning=FALSE}
# TODO: do it for each variable

# Check data
head(si_df_final_m)

# Identify variables
vars <- unique(si_df_final$thisVar)
...
stCol <- 7 # FIXME: Find a way to select the initial column autimatically

for (v in 1:length(vars)) {
  
 for(o in stCol:length(outNames)){
   
  df_temp <- si_df_final_m %>% subset (thisVar == vars[v])
  head(df_temp)
...
group_by(pixel_ID, Combine) %>%
  select(TMean, TotalBiomass, HarvestIndex, Combine) %>%
