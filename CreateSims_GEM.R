...
hybrids <- c("h1","h2", "h3","h4")                                               # select hybrids
p1 <-  c(120,153,187,220) # parameter values for emergence to end of juvenile
p2 <- c(850,883,917,950) # parameter values for flowering to maturity
...
for(cl in 1:length(climates)){
  ...
  #loop through met file names to create one .sim file for each met file
    ...
    lapply(nodesStSow, function(n) {
      xmlValue(n) <- paste0(15,sowDate[s]) # just assumes starting with 1st (or 15th) day and moving a day ahead for others
    })
    lapply(nodesEnSow, function(n) {
      xmlValue(n) <- paste0(16,sowDate[s])
    })
    lapply(nodesEnRot, function(n) {
      xmlValue(n) <- paste0(17,sowDate[s])
    })
