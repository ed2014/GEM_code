...
```{r SetOutFolder}
# set output folder
outFolder <- paste0(climDir,"/metFiles_",RCP,"_",runPeriod,"/")
...
for (y in 1:length(firstYear)) {
  ...
  # open netCDF
  base.nc <- open.ncdf(paste0(climDir,"\\",thisFile))
  
  # loop daily to get doy and year dimensions
  for (t in 1:length(get.var.ncdf(base.nc, "time"))) {       
 ...
 #   c <- 130
    ...
    for (f in 1:length(factorName)) {            
      ...
      for (y in 1:length(firstYear)) {              
        ...
        netc <- open.ncdf(paste0(climDir,"//",fileName))
        
        time.dim <- length(get.var.ncdf(netc, "time")) # FIXME: this is same as base? (delete it?)
        thisLat <- netc$dim$latitude$vals[r]
        thisLong <-  netc$dim$longitude$vals[c] 
        
        # Do stuff with open netCDF file
        myVar <- NULL
        myVar <- get.var.ncdf(netc, start=c(c,r,1), count=c(1,1,-1))   
